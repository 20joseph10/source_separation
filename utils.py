...
def get_batch_spec(mixed, s1, s2, sample_frames=10, batch_size=64):

	mixed_batch = list()
	s1_batch = list()
	s2_batch = list()
	data_len = len(mixed)
	random_sample_idx = np.random.choice(data_len, batch_size, replace=True)
	...
	for idx in random_sample_idx:
		...
		num_frames = mix_.shape[1] # time-step
		start = np.random.randint(num_frames-sample_frames+1)
		end = start + sample_frames

		mixed_batch.append(mix_[:, start:end])
		s1_batch.append(s1_[:, start:end])
		s2_batch.append(s2_[:, start:end])
	...
	mixed_batch = mixed_batch.transpose((0,2,1))
	s1_batch = s1_batch.transpose((0,2,1))
	s2_batch = s2_batch.transpose((0,2,1))

	return mixed_batch, s1_batch, s2_batch
